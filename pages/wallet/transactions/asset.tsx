...
import { ApiAssetTransaction, ApiPagination } from "types";
...
export function AssetTransactions() {
  ...
  const { data, error, isLoading } = useSWR(
    `/api/transactions/asset?page=${page}&limit=${pageSize}`,
    (url) =>
      axiosInstance.user
        .get<ApiPagination<ApiAssetTransaction>>(url)
        .then((res) => res.data)
  );
  ...
  const columns: ColumnsType<ApiAssetTransaction> = [
    ...
    },
    {
      title: "Type",
      dataIndex: "type",
      key: "assetTransactionType",
      render: (_, t) => <Tag>{t.assetTransactionType.toUpperCase()}</Tag>,
    },
    {
      title: "Date & Time",
      dataIndex: "createdAt",
